...
class Conv(NN):
    ...
    def __init__(self, layers_n=9, layer_7_filters_n=40, activation=tf.nn.sigmoid, use_softmax=True, cross_entropy=False):
        ...
        NN.__init__(self, 0.01, 0.99, self.TFRECORD_SUFFIX, 50, 50, 3, loss if cross_entropy else None)
    ...
    def _create_nn(self):
        ...
        current = tf.layers.conv2d(current, 24, 3, activation=self.activation)
        current = tf.layers.max_pooling2d(current, 2, 2)
        current = tf.layers.conv2d(current, 32, 5, activation=self.activation)
        ...
        last_conv_output_size = last_conv_output_shape[1] * last_conv_output_shape[2] * last_conv_output_shape[3]

        current = tf.layers.dense(tf.reshape(current, [-1, last_conv_output_size]), 100, activation=self.activation)
